...
data.resume_after (String) [optional] ::
  ...
  event will have a different event ID in its "resume_after" key value.
...
In case the `resume_after` has been given during subscription, resume info is
...
* 3000 : Authentitcation error
* 3001 : Message history not supported
